...
public partial class Pages_ToUser_MinhasCaravanas : System.Web.UI.Page
{
    ...
    {
        DataSet ds = CaravanDB.selectCaravanCriadorbyEmpresa(Convert.ToInt32(Session["id"]));
        string ltl_status = "";
        ...
        if (!plh_status.HasControls())
        {
            ...
            ltl_status += "  <h5 class='text-center'>Você não realizou nenhuma caravana</h5>";
           ltl_status += "</div>";
            none.Text = ltl_status;
            plh_status.Controls.Add(none);
        }

         ds = CaravanDB.selectCaravanCriadorbyEmpresa2(Convert.ToInt32(Session["id"]));
        string aux;
        if (ds.Tables[0].Rows.Count > 0)
        {
            aux = ds.Tables[0].Rows[0]["id"].ToString();
        ...
        if (!plh_status2.HasControls())
        {
            ...
            ltl_status = "";
            ltl_status += "    <div class='col - 12 mt-2'>";
            ltl_status += "  <h5 class='text-center'>Não há nenhuma Caravana Agendada</h5>";
            ltl_status += "</div>";
            none.Text = ltl_status;
